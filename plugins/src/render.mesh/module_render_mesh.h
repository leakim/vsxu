...
class vsx_module_render_mesh : public vsx_module
{
  ...
  bool handle_gl_error( char* filename, int file_line)
  {
    //return true;
    GLenum gl_error;
    gl_error = glGetError();
    ...
    {
      printf("%s GlGetError()=%d on line %d\n",filename,gl_error,file_line);
      gl_errors.push_back("Gl error on line "+i2s(file_line)+" error code: "+i2s(gl_error));
      //exit(20);
      return false;
    }
    return true;
